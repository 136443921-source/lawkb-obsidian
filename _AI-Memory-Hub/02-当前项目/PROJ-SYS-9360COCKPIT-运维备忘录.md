...
| 日期 | 变更 |
|---|---|
| 2026-09-17 22:15 | **C4-2 单案看板③证据清单满配（6658/6660 真卷实地转录）**：从桌面案卷 `~/Desktop/小强律师办案系统/庭审10件套_丁戊祥诉刘九零吊顶案（6658）/02_举证清单.docx` 与 `庭审10件套_雅菲软装窗帘案（6660）/02_举证清单_雅菲软装窗帘案_最新版.docx` 用 python zipfile+ElementTree 实拉证据表（非凭记忆），严格转录 11 项/案（`{no,name,source,purpose}` 契约）注入 `index.html` 内联 DATA + `data.js` 双源。**修掉 6660 证据空白真因**：原 6660 的 evidence 挂在 8 空格嵌套层级（非 case 直接属性），`c.evidence` 读不到——两文件均已移至 6 空格直接属性层。注入脚本两轮修复：①对象尾逗号缺失致 JS 语法错（从备份恢复重跑）；②Python eval 校验不认 JS `null`，改用 Node 权威校验（data.js 整体 eval + index.html DATA 括号配对抽取）。验证：Node 校验两文件均 6658=11/6660=11 项且无缺失字段；浏览器 E2E 两案证据表均 11 行渲染（截图实证：报价单/收据/微信记录/订购协议/情况说明等真卷内容）。⑦委托协议⑧费用桌面无源文件，按"不编造"铁律保持待录入。备份 `/tmp/clm_evidence_inject_20260917-161407/`。 |
| 2026-09-17 | **C4 双看板重构（对标 Harvey Command Center）**：① tab 改为「C4-1 全量看板 / C4-2 单案看板 / 当事人可视化报告（脱敏）」三态；② C4-1 聚焦「每案待办事项汇总（13 张卡：未决期限+下一步动作摘录）+ 期限倒计时榜（已办结折叠计数）」，原全量案件态势卡/智能体挂载区撤出；③ 新增 C4-2 单案看板：案号/ID/关键词输入查询（Enter 回车+快捷案卡+datalist 联想），九段式单案全景（①案件概况②事实脉络③证据清单及证明目的④阶段进度⑤案件态势⑥下一步动作⑦委托协议及期限⑧费用缴纳⑨当事人风险提示）+ 附加 CLM 智能体路由/模拟法庭入口；evidence/retainer/fees 为可选数据契约字段，缺省显示待录入引导；④ **修复重要 bug**：tab 切换原用内联 style.display，顶不过 `.hide{display:none!important}`，导致 C4-2/当事人 tab 内容区实际隐形（此前 E2E 只查 DOM 数未查 computed visibility 漏网）——切换器改 classList.toggle + senderSection 补初始 hide；E2E 以 computed display 矩阵 + 三 tab 截图验证通过。 |
| 2026-09-17 14:46 | **微信扫码打不开·根因修复（老强反馈实测）**：根因=发送台生成链接写死 `127.0.0.1`（本机回环地址）且服务器仅监听 localhost，手机微信扫后访问的是手机自己→打不开。三处修复：① 服务器由 launchd 守护 `com.xiaoqiang.cockpit.server.plist` 拉起，原 host=`127.0.0.1`，改为 `0.0.0.0` 并 `launchctl unload/load` 重载（PID 27487 接管、KeepAlive 持久化，重启电脑自动拉 0.0.0.0 版）；`start_cockpit_hub_ops.sh` 的 `HOST` 同步改 `0.0.0.0`；② 发送台新增「分享链接前缀」输入框（默认=当前页目录，从 127.0.0.1 打开即 127.0.0.1、从 LAN IP 打开即 LAN IP）+「填入本机IP」按钮（WebRTC 探测本机局域网 IP 自动填充，失败提示手动填）+ 回环告警（前缀仍 127.0.0.1/localhost 时点生成链接即提示"手机无法访问，请先点填入本机IP"）；③ 新增「导出报告文件」按钮，调用 `report-render.js` 的 `window.buildStandaloneReport()` 生成完全自包含 HTML（内联 STYLE+该案渲染结果，7.4KB，无外部依赖），下载后发微信/邮件、对方离线打开，最稳、不依赖服务器与网络、契合"数据不出所"。验证：Node 测 buildStandaloneReport 输出含 DOCTYPE/creport/style、0 内部泄漏、案号脱敏；浏览器 E2E 以 LAN IP 开 index.html 生成链接=`http://192.168.2.83:9360/subapps/clm/report.html?case=6658`、二维码含 LAN IP、回环告警生效；双地址(127.0.0.1 与 192.168.2.83)均 200。 |
| 2026-09-17 14:40 | **C4 当事人只读子页升级为「当事人可视化报告」+ 一键推送端口（老强指令，参照 Harvey Command Center）**：① 新建 `subapps/clm/report-render.js` 单一模板源，六段式报告（①执行摘要 ②里程碑时间线 ③当前阶段进度 ④本期关键进展 ⑤下一步动作 ⑥费用与期限+风险提示），`index.html` 的 `#viewClient` 子页改由其统一渲染；② 新建独立推送页 `report.html?case=<caseId>`（自包含，微信可直接打开），发送台「生成链接」一键产出该案链接 + 二维码（qrserver 联网时显示，离线链接仍可用）；③ 脱敏四层加固：案号打码 `****号`、内部字眼过滤（timeline/nextActions/publicView.done/next/stageText 全量过 `isClientSafe` 十词黑名单，修复 6660 `pub.next`「自认钉死…」泄漏）、委托人姓名统一替换「您方」（`c.client` 逐名替换）、风险敞口/智能体挂载/发送台不渲染；④ **修复本轮回归根因**：`isClientSafe` 原按字符串设计，收到 timeline 事件对象即抛 `t.indexOf is not a function`，report.html try/catch 吞错→整页 0 块静默白屏；已新增 `safeText()` 兼容对象入参；⑤ 验证：Node DOM-stub 全 13 案 PASS（1 块/0 内部泄漏/0 姓名泄漏/案号全脱敏）；浏览器 E2E `?case=6658` 六段式渲染正常（截图实证）。⑥ `data.js` 仅由 report.html 使用，index.html 保留内联 DATA（两者 generatedAt 同为 2026-09-17 13:20 已核一致）。备份 `/tmp/clm_data_extract_20260917-134835/`（改前）、`/tmp/clm_report_final_20260917-143945/`（终态）。 |
| 2026-09-17 00:50 | **登录卡片副标题替换为律所口号**：`index.html` 主登录卡片 `.sub` 副标题第一行由屏清单「合同生命周期中台 · 模拟法庭 · 车机 · 知识飞轮 · 决策思维树 · 积分监测 · 案件生命周期 · AI 幻觉监控」替换为所训口号 **「数据不出所、人走资产在、经验可沉淀、带教成长快」**；下方「统一作战地图评分导航首页 · 受角色权限控制」鉴权提示行保留。浏览器端到端实测登录窗口副标题渲染正确、旧屏清单 0 残留。备份 `/tmp/cockpit_login_slogan_20260917-004721/`。 |
| 2026-09-16 23:36 | **用户徽标改「登录账户 · 真实姓名」**：统一演示账号 `rbac.js` 的 `name` 由「演示账号（主任凭据·全席位通用）」改为 **「LZ-001 · 陈友强」**（经 AskUserQuestion 确认方案：统一账号+真实姓名，四席位共用天然同步）；缓存参数 bump `?v=20260916d`。端到端实测：主任/执业律师席位徽标均显示「LZ-001 · 陈友强」，律师 12 屏不受影响。备份 `/tmp/cockpit_badge_name_20260916-233636/`。 |
...
## 相关笔记
- [[PROJ-SYS-LAWKB-知识飞轮运维]] (共现关键词: SYS, PROJ, 运维)
- [[PROJ-SYS-9655COCKPIT-单机驾驶舱]] (共现关键词: SYS, PROJ)
- [[cockpit_monitor_2026-09-16]] (共现关键词: 2026, 09, 证据)
- [[_2026-09-16-D6-驾驶舱监控技能建设]] (共现关键词: 2026, 09, 16)
- [[DEC-2026-016-9655单机驾驶舱1比1复原9360框架]] (共现关键词: 2026, cockpit)
