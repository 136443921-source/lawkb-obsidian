...
用法：python _fix_deadlinks_v7_20260915.py [--apply]
"""
...
def main():
    ...
    files = gate.md_files()
    # ── 坑 61 并发保护：跳过「近期被改」文件，避免与他会话写入互冲 ──
    import time
    RECENT_SEC = int(os.environ.get("DEADLINK_RECENT_SEC", "1800"))   # 默认 30 分钟
    now = time.time()
    recent = set()
    for p in files:
        try:
            ...
            pass
    if recent:
        print(f"⚠️  并发保护：近 {RECENT_SEC//60} 分钟内被改的 {len(recent)} 个文件，"
              f"本次只报不改（坑 61，防撞他会话写入）")
        for p in sorted(recent)[:15]:
    ...
    out = {"generated": STAMP, "basename_set": len(names),
           "concurrent_guard_sec": RECENT_SEC,
           "recent_files_skipped": sorted(os.path.relpath(p, ROOT) for p in recent),
           "A_safe": A, "B_candidate": B, "C_manual": C, "D_missing_num": D}
    op = os.path.join(HERE, f"死链分级-20260915.json")
    ...
    if APPLY and A:
        ...
        for dead, info in A.items():
            for p in info["files"]:
                if p in recent:
                    skipped += 1
                    print(f"  ⏭  跳过（并发保护）：{os.path.relpath(p, ROOT)}")
                    continue
                ...
                shutil.copy2(p, os.path.join(
                    BK, os.path.relpath(p, ROOT).replace("/", "__")))   # 扁平版，回滚白名单两形态齐全
                open(p, "w", encoding="utf-8").write(t.replace(dead, info["to"]))
                hit += t.count(dead)
        if skipped:
            print(f"  ⏭  并发保护跳过 {skipped} 个文件（未改）")
