...
"""
第六部分 抚养纠纷 · 审判要件卡生成器
起始号：R-HY-056（HY 域物理 max=055，2026-09-07 实地取号）
"""
...
LIB = json.load(open(os.path.join(BASE, "02-提炼/审判要件卡/贵州类案指南第二卷-抚养纠纷-法条回填库.json"),
                     encoding="utf-8"))
...
CARDS = load("fy", os.path.join(D, "_fy_cards_20260908.py")).CARDS
...
LAWNAME_MAP = {
    ...
    "诉讼费用交纳办法": "《诉讼费用交纳办法》",
    "审理精神损害赔偿案件解释": "《最高人民法院关于确定民事侵权精神损害赔偿责任若干问题的解释》",
}
...
def build(c, cardmap):
    ...
    pdf = c["pdf"]
    bp = bookpage(pdf)
    chapter = c.get("chapter", "第四章 审理思路")
    src = "《贵州法院类案审判要件指南（第二卷）》第六部分 抚养纠纷案件审判要件指南 %s（PDF页%s（书页%s））" % (
        chapter, pdf, bp)
    related = [cardmap[j] for j in c.get("related", []) if j in cardmap]

    L = []
    L.append("---")
    L.append("title: %s" % yq(c["title"] + "（审判要件卡·抚养纠纷）"))
    L.append("rule_id: %s" % r)
    L.append("card_type: 审判要件卡")
    L.append("source: %s" % yq(src))
    L.append("type: 抚养纠纷·审判要件卡")
    ...
    L.append("- 卡内法条正文均经权威源核填（北大法宝核填；解释二§18/民法典§1073 加华宇元典交叉核验），标注现行有效。")
    L.append("")
...
def main():
    ...
    print("互链解析：%d/%d 张卡片含 related" % (sum(1 for c in CARDS if c.get("related")), len(CARDS)))
    if not apply:
        print("\n[dry-run] 未写入。加 --apply 落盘。")
        return
    if conflict and "--force" not in sys.argv:
        print("存在冲突，已中止写入（安全铁律）。加 --force 可覆盖同批卡片。")
        return
    n = 0
    for c in CARDS:
