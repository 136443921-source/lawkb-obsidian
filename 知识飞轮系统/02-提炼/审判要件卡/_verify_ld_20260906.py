...
# -*- coding: utf-8 -*-
"""
新就业形态劳动争议审判要件卡 交付校验器（2026-09-06）
...
CARD_DIR = os.path.join(BASE, '06-沉淀/裁判规则库/劳动人事')
LINK_DIRS = [
...
NUMBERS = list(range(10, 46))  # R-LD-010 ~ R-LD-045
...
files = {}
for n in NUMBERS:
    pat = os.path.join(CARD_DIR, f'R-LD-{n:03d}-*.md')
    got = glob.glob(pat)
    if len(got) == 0:
        E('E3', f'缺卡 R-LD-{n:03d}')
    elif len(got) > 1:
        E('E3', f'编号 R-LD-{n:03d} 存在 {len(got)} 个文件（重复）')
    else:
        files[n] = got[0]
...
for n, path in sorted(files.items()):
    ...
    rid = fm.get('rule_id')
    if rid != f'R-LD-{n:03d}':
        E('E3', f'{fn}: rule_id={rid} 与文件名不符（应为 R-LD-{n}）')
    if rid in seen_ids:
    ...
    if not m4:
        ...
        for b in law_blocks:
            ...
            if ('元典核填' not in b) and ('本地·' not in b) and ('权威源核填' not in b) and ('法宝核填' not in b):
                E('E8', f'{fn}: 法条块「{title_line[:40]}」未标权威源（元典核填/本地·/权威源核填/法宝核填）')
