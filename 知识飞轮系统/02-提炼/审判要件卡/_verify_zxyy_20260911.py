...
OUTDIR = '/Users/chenyouqiang/Documents/LawKB/知识飞轮系统/06-沉淀/裁判规则库/案外人执行异议之诉'
MAP = json.load(open('/Users/chenyouqiang/Documents/LawKB/知识飞轮系统/02-提炼/审判要件卡/贵州类案指南第二卷-页码映射表.json', encoding='utf-8'))
PREFIX = [f'R-PR-{n:03d}' for n in range(69, 84)]

ERRORS, WARNS = [], []
files = sorted(glob.glob(f'{OUTDIR}/R-PR-*.md'))
cand = [f for f in files if os.path.basename(f)[:8] in PREFIX]
...
for f in cand:
    ...
    for lnk in (d.get('related_links') or []):
        if isinstance(lnk, str) and lnk.startswith('R-'):
            if not glob.glob(f'/Users/chenyouqiang/Documents/LawKB/知识飞轮系统/**/{lnk}.md', recursive=True):
